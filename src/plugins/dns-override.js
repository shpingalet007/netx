...
export class DnsOverrideList extends PluginList {
  ...
  getAllAddresses(host) {
    ...
    if (ipV4?.length) {
      ipV4.forEach((ip) => ipV4List.push({ ip, family: 4 }));
    }

    if (ipV6?.length) {
      ipV6.forEach((ip) => ipV6List.push({ ip, family: 6 }));
    }
  ...
  getAddress(host, family = 4) {
    ...
    const isHostAddressEmpty =
      !this.list?.[host]?.ip?.v4?.length && !this.list?.[host]?.ip?.v6?.length;
    ...
    if ((family === 0 || family === 4) && addr4)
      return { ip: addr4, family: 4 };
    if ((family === 0 || family === 6) && addr6)
      return { ip: addr6, family: 6 };
